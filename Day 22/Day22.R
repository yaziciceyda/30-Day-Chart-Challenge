...
first_plot <- ggplot(stations_data, aes(state = state, fill = freq)) +
  ...
  theme(panel.background = element_rect(fill = "black", color = NA),
        ...
        legend.title = element_text(family = "electrolize", size = 13,
                                    hjust = 0),
        legend.text = element_text(family = "electrolize", size = 13),
...
subtitle_text <- str_wrap("The number of alternative fuel stations in the United States started to increase after
...
caption <- ggdraw() + 
  draw_label(
    "Data Source: US DOT | #30DayChartChallenge - Day #22 | TidyTuesday 2022 - Week 9 | Prepared by: C. YAZICI", 
    fontface = 'bold',
    fontfamily = "electrolize",
    hjust = 0.5,
    x = 0.5,
...
ggsave("Day22.png", final_plot, width = 25, height = 12, dpi = 72)
