...
p <-  ggplot()+
  geom_polygon(data_sherlock, mapping = aes(y = avg_rating, x  = personality,
                                  group = char_name),
               fill = "black", color = "black", alpha = 0.35)+
  ...
  geom_image(data_sherlock, mapping = aes(x = 1, y = -40, 
                                          image = image_cropped), size = 0.3) +
  geom_richtext(data_sherlock, mapping = aes(label = label, x  = personality,
                                             y = 130),
                family = "Cabin Sketch", 
                colour = "black",
  ...
  labs(title = "\n",
       ...
       y = "",
       tag = "SHERLOCK\nHOLMES",
       caption = "Data Source: Open-Source Psychometrics Project | #30DayChartChallenge - Day #16 | TidyTuesday 2022 - Week 33 | Prepared by: C. YAZICI")+
  theme_minimal()+
  theme(plot.background = element_rect(fill = "#d0c4b4",color = NA),
        ...
        plot.caption = element_text(color = "black",size = 21, 
                                  hjust = 1, family = "Cabin Sketch"),
        text = element_text(color = "black"),
        plot.tag.position = c(0.75, 0.3),
        plot.tag = element_text(family = "Cabin Sketch", size = 100, 
                                face = "bold"))
p

# Save the Plot

ggsave("Day16.png", p, width = 27, height = 15, dpi = 72)
